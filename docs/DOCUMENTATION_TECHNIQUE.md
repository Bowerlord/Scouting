...
- **Data Refresh GitHub Actions** (`.github/workflows/data-refresh.yml`) : chaque lundi et jeudi (cron) ou à la demande, ré-exécute le pipeline complet avec les dépendances complètes (xgboost + umap), contrôle le résultat dans le même job (ruff, pytest, banc de l'agent avec `SCOUTING_REFRESH_SNAPSHOTS=0` pour que l'agent lise les fichiers régénérés) et pousse les snapshots de `reports/metrics/` directement sur `main`. En cas d'échec (ID Drive expiré, dérive de schéma, contrôle rouge), rien n'est publié et l'issue de suivi est commentée avec le lien du run.
